...
def process_input(in_f):
    ...
    for index, row in df_in.iterrows():
        ...
        if row["old_or_new"] == 'old':
            old_pairs['All'].append(num_pair)
            old_pairs[row['category']].append(num_pair)
        else:
            new_pairs['All'].append(num_pair)
            new_pairs[row['category']].append(num_pair)
...
def create_instruction(testing_pairs, out, ftype):
    for t in testing_pairs.keys():
        ...
        num_trial, i = 0, 0
        for num_pair in testing_pairs[t]:
            target, foil = num_pair
            # Convert number to word
            # Target
            target_word = num2words(target).replace(' and', '')
            target_word = target_word.replace(',', '')
            target_word = target_word.replace('-', ' ')        
            # Foil
            foil_word = num2words(foil).replace(' and', '')
            foil_word = foil_word.replace(',', '')
            foil_word = foil_word.replace('-', ' ')
